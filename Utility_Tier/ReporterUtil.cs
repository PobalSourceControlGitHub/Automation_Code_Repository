...
namespace Automation_Suite.Utility_Tier
{
    class ReporterUtil : ReportsGeneration
    {
        ...
        public static int intFailTCCount = 0;
            ...
            if (!Env.boolDebugFlag) ReportsGeneration.ExtentReportStart();
            ...
            {
                if (!Env.boolDebugFlag) _test = o_ExtentReports.CreateTest(TestName, TestDescription);
            ...
            {
                if (!Env.boolDebugFlag) _test.AssignCategory(TestName, TestDescription);
            ...
            public static void ReportEvent(string staus, string strStepName, string strException)
            {
                ...
                if (!Env.boolDebugFlag)
                {
                    ...
                    if (staus.ToLower() == Env.Pass) _test.Log(Status.Pass, strStepName + strException);
                    if (staus.ToLower() == Env.Info) _test.Log(Status.Info, strStepName +strException);
                    if (staus.ToLower() == Env.Warning) _test.Log(Status.Warning, strStepName +strException);
                    if (staus.ToLower() == Env.Fail)
                    {
                    
                    DateTime time = DateTime.Now;
                    string fileName = "Screenshot_" + time.ToString("h_mm_ss") + ".png";
                    string screenShotPath = Capture(GetDriver(), fileName, false);
            ...
            // Author: Ankita
            public static string CaptureScreenShot(string strScreenshotPath)
            {
                try
                {
                object screen = null;
            ...
            DataTable oTable = ExcelUtil.ExcelToTable(@"C:\Pobal_AutomationProject\Pobal_Test_Project\Automation_Suite\Configuration_Tier\Configuration.xlsx", Env.Env_Config);
